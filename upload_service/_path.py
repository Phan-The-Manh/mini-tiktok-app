...
The upload service depends on two things from the Database Layer:

    from client import get_mongo, get_redis, get_minio, get_minio_bucket_name
    from schemas import Video, ModerationStatus, DistributionStage
...
upload_service can then `import _path` before importing from the Database Layer.
...
# upload_service/ and database/ are siblings under the project root.
...
# Then layer upload_service/.env on top (so it can override per-service settings).
